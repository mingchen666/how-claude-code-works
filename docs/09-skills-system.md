...
`whenToUse` 字段是技能被模型自动发现的关键。它出现在技能列表中，模型据此判断"当前场景是否需要调用这个技能"。
...
| **扩展** | `hooks` | 技能级 Hook 定义（详见 [9.8](#98-扩展机制与设计洞察)） |
...
技能内容在执行时经过多层替换（`getPromptForCommand()`），每一层解决一个具体问题：
...
所有嵌入的 Shell 命令会**并行执行**（`Promise.all`），每个命令执行前都会进行权限检查。MCP 技能来自远程不受信任的服务端，因此跳过 Shell 执行和 `${CLAUDE_SKILL_DIR}` 替换——这是安全关键路径上的显式检查，在 [9.6 节](#96-安全与信任模型)详细分析。

## 9.5 执行模型：Inline vs Fork
