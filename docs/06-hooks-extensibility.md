...
- **6.1 事件全景**：27 种 Hook 事件的分类与触发时机
- **6.2 Hook 类型**：4 种可配置 Hook（Command/Prompt/Agent/HTTP）+ 2 种编程式 Hook（Callback/Function）
- **6.3 Matcher 匹配器**：三级匹配机制与 `if` 条件的配合
- **6.4 执行引擎**：6 阶段流水线——信任检查、匹配、去重、并行执行、输出解析、结果聚合
- **6.5-6.9 高级主题**：JSON 输出协议、信任模型与安全、PermissionRequest 深度解析、Stop Hook、实战模式
